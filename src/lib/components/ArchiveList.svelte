<script>
  ...
  const crosswordDates = Object.keys(crosswords)
    .filter((date) => {
      ...
      return crosswords[date]?.words?.some(
        (word) => word.word !== "X" && word.audioUrl !== "000"
      );
    })
    .sort();
  ...
  function goToPreviousMonth() {
    ...
    const earliestMonth = new Date(
      earliestCrosswordDate.getFullYear(),
      earliestCrosswordDate.getMonth(),
      1
    );
...
<div class="container max-w-md mx-auto md:mt-6">
  <div class="bg-white dark:bg-white shadow rounded-lg overflow-hidden">
    <div
      class="bg-gray-100 dark:bg-black dark:text-white px-1 py-1.5 border-b flex items-center justify-between"
    >
      <button
    ...
    <div
      class="dark:text-black grid grid-cols-7 text-center font-medium text-black-300 p-2"
    >
      ...
      {#each generateCalendarDays() as dayObj}
        ...
        {:else}
          <button
            ...
            class:bg-green-500={selectedDate !== dayObj.date &&
              solvedPuzzles.includes(dayObj.date)}
            class:hover:bg-green-600={solvedPuzzles.includes(dayObj.date)}
            class:bg-gray-200={selectedDate !== dayObj.date &&
              dayObj.isPuzzleAvailable &&
              !solvedPuzzles.includes(dayObj.date)}
            class:hover:bg-gray-300={selectedDate !== dayObj.date &&
              dayObj.isPuzzleAvailable &&
              !solvedPuzzles.includes(dayObj.date)}
            class:hover:bg-black={selectedDate !== dayObj.date &&
              dayObj.isPuzzleAvailable}
            class:hover:text-white={selectedDate !== dayObj.date &&
              dayObj.isPuzzleAvailable}
            class:text-black={selectedDate !== dayObj.date &&
              dayObj.isPuzzleAvailable &&
              !solvedPuzzles.includes(dayObj.date)}
