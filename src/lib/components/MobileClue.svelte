...
{#if clue}
  <div class="fixed bottom-[180px] left-0 right-0 h-14 bg-white border-t border-gray-200 shadow-lg mb-4">
    <div 
    ...
    >
      ...
      <button
        onclick={() => onPlay(clue)}
        class="w-11 h-11 flex items-center justify-center rounded-full bg-black text-white hover:bg-gray-800 disabled:opacity-50"
        disabled={isPlaying}
      >
        {#if isPlaying}
